...
public class RetanguloGr extends Retangulo{
     ...
     * Desenha um retangulo a partir de 2 diagonais, utilizando a equacao da reta
    ...
    public void desenharRetangulo(Graphics g){  
        ...
        int x3 = (int)p2.getX(); //Diagonal 3
        int y3 = (int)p2.getY();
        
        int x4 = (int)p2.getX(); //Diagonal 4
        int y4 = (int)p2.getY();
         
        RetaGr r = new RetaGr(x1, y1, x2, y2, getCorRetangulo(), getNomeRetangulo(),  getEspRetangulo());
        if(x1 == x2 || y1 == y2){ //Uma unica reta
            //r.setP1(p1);
            //r.setP2(p2);
            r.desenharReta(g);  
        }else{
